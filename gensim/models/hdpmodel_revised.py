    ...
    * whole corpus processed
    ...
    def update(self, corpus):
        ...
        * whole corpus processed
        ...
        ##### get the n nearest neighbors (check spatial LDA)
        ##### create a j by n by t matrices to store the values for each estimated ones
        ##### put the phi (zeta) from teh neighbors as the intital phi
        ...
        for doc in chunk:
            if len(doc) > 0:
                ...
                doc_score = self.doc_e_step(
                    ...
                    doc_word_counts, self.m_var_converge, (self.m_num_docs_processed+idx_chunk)########
                )
    ...
    def doc_e_step(self, ss, Elogsticks_1st, unique_words, doc_word_ids, doc_word_counts, var_converge, doc_idx):
        ...
        while iter < max_iter and (converge < 0.0 or converge > var_converge):
            ...
            # var_phi
            if iter < 3:
                var_phi = np.dot(phi.T, (Elogbeta_doc * doc_word_counts).T) ##### change this part
                (log_var_phi, log_norm) = matutils.ret_log_normalize_vec(var_phi) ##### change this part
